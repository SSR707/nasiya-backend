...
export class PaymentController {
  ...
  @ApiParam({name: 'type', description: 'Payment type (CASH, CARD, BANK_TRANSFER)'})
  async getPaymentByType(@Param('type') type: 'CASH' | 'CARD' | 'BANK_TRANSFER') {
  ...
  async getPaymentsBetweenDates(@Query('startDate') startDate: string, @Query('endDate') endDate: string) {
    return await this.paymentService.findPaymentsBetweenDates(startDate, endDate);
  }
  ...
  @ApiQuery({ name: 'newType', description: 'New payment type (CASH, CARD, BANK_TRANSFER)', required: true })
  async updatePaymentType(@Param('id') id: string, @Query('newType') newType: 'CASH' | 'CARD' | 'BANK_TRANSFER') {
  ...
  @ApiParam({ name: 'debtId', description: 'Debt ID related to payments to be deleted' })
