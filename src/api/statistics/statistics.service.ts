...
export class StatisticsService {
    ...
    async calculateTotalDept(): Promise<number> {
        ...
        const totalDept = depts.reduce((sum, dept) => sum + Number(dept.debt_sum), 0)
        return totalDept
    }
    ...
    async getActiveDeptors(): Promise<DebtorEntity[]> {
        return await this.deptorRepository.find({
            ...
            select: ['id', 'full_name', 'phone_number', 'image', 'address', 'note', 'is_active']
        })
    }

    async calculateDailyDept(): Promise<number> {
        const dailyDebt = await this.paymentRepository.find()
        const totalDailyDebt = dailyDebt.reduce((sum, payment) => sum + Number(payment.sum), 0)
